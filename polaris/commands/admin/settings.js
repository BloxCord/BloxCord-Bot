...
async function setMainGroupId (msg, settings, client) {
	...
	if (group.error) {
		if (group.error.status === 404) {
			// Group not found. All other HTTP errors logged by BloxCord-RBX
...
async function changePrefix (msg, settings, client) {
	...
	if (success) {
		return msg.channel.sendSuccess(msg.author, {title: 'Successfully updated the prefix', description: `The new command prefix for this server is \`${res.content}\`! Users should use this new prefix to interact with BloxCord.`});
	} else {
		return msg.channel.sendError(msg.author, {title: 'Oops! Something went wrong.', description: `Something went wrong with the database. Please try again.`});
	}
}
